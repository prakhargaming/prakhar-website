  ...
  return (
    <div className="flex md:flex-row justify-center md:items-center w-full h-full max-sm:flex-col">
      <div className="max-sm:py-5 md:w-1/3 pr-9 md:text-right max-sm:text-left">
        <h1
          className={`text-4xl max-sm:text-2xl font-bold mb-4 md:pl-8 ${isDarkMode ? "text-white" : "text-black"}`}
        >
          Sign up to receive updates!
        </h1>
        <h2
          className={`md:text-xl max-sm:mb-4 max-sm:text-md max-sm:text-left md:mb-4 md:pl-8 max-sm:w-full max-sm:max-h-[15vh] overflow-y-auto ${isDarkMode ? "text-white" : "text-black"}`}
        >
          Here, you can sign up to recieve email notifications for stuff that is
          going with the website and updates regarding my career.
        </h2>
